import React from 'react'
import { ArrowRight } from 'react-feather'

function Hero() {
  return (
    <div name='home' className='bg-[#08192f] w-full h-screen'>
       <div className='max-w-[1000px] mx-auto px-3 flex flex-col justify-center h-full'>
            <p className='text-[#8892b0]'>Hi, My Name is</p>
            <h1 className='text-4xl sm:text-7xl font-bold text-[#ccd6f6]'>Kehinde Balogun</h1>
            <h2 className='text-4xl sm:text-7xl font-bold text-[#8892b0]'>I'm a Frontend Developer</h2>
            <p className='text-[#8892b0] py-4 max-w-[700px]'>
                I'm a Frontend web developer specializing in HTML, CSS, Vanilla JS, and reactJS. Currently,
                I'm Focused on building responsive web applications, 
                I'm also currently enrolled in AltSchool of Engineering.  
            </p>
            <div>
                <button className='text-white group border-2 px-6 py-3 my-2 flex items-center hover:bg-pink-600 hover:border-pink-600'>
                    View Projects
                    <span  className='group-hover:rotate-90 duration-300'>
                        <ArrowRight className='ml-3'/>
                    </span>
                </button>
            </div>
        </div>        
    </div>
  )
}

export default Hero